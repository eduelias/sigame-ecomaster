...
YAHOO.widget.DataTable.formatHOTSig = function(el, oRecord, oColumn, oData){
	var elTr = el; //.parentNode;
	if (oData <= 100){	 
		classe = "icon-boneco";
	} else if ((oData > 100) && (oData <= 150)){
		classe = "icon-alert";
	} else {
		classe = "icon-x";
	}
	var id = oRecord._oData.id;
	var url = 'modules/significancia/notas.php?ID='+WBS.MID+'_'+id;
	_hander = {
		success:function(o){
			arr = eval(o.responseText);
			...
			if (arr) {
				...
				for (i=1;i<10;i++){
					...
					if (aux) {
						...
						if (ar.SOMA == 0){	
							classe = "icon-traco";
						} else if ((ar.SOMA > 0) && (ar.SOMA <= 100)) {
							classe = "icon-boneco";
						} else if ((ar.SOMA > 100) && (ar.SOMA <= 150)){
							classe = "icon-alert";
						} else {
							classe = "icon-x";
						}
			...
			el.innerHTML = '<div class='+classe+' style="font-size:14px">&nbsp;</div>';
			var ttip = new YAHOO.widget.Tooltip("tt"+id,{context:el.firstChild, text: tx});
	...
	YAHOO.util.Connect.asyncRequest('GET',url,_hander, null);
	//alert(elTr.class);
	el.innerHTML = '<div class='+classe+' style="font-size:14px">&nbsp;</div>';
}

YAHOO.widget.DataTable.formatAlerta = function(el, oRecord, oColumn, oData){
	if (oData == 0) {
		classe = "";
	} else if ((oData > 0) && (oData <= 100)){	 
		classe = "icon-boneco";
	} else if ((oData > 100) && (oData <= 150)){
		classe = "icon-alert";
	} else {
...
YAHOO.widget.DataTable.formatSig = function(el, oRecord, oColumn, oData){
	var elTr = el; //.parentNode;
	if (oData <= 100){	 
		classe = "icon-boneco";
	} else if ((oData > 100) && (oData <= 150)){
 ...
 * @static
 */
YAHOO.widget.DataTable.formatLegal = function(elCell, oRecord, oColumn, oData) {

	elCell.innerHTML = '<div class="icon-info">&nbsp;</div>';

	YAHOO.util.Event.addListener(elCell, 'mouseover', function(e, oSelf){
															   
				WBS.central.setHeader(oRecord._oData['label']);   
				WBS.central.setBody('<p align="justify">'+oRecord._oData["descricao"]+"</p>");	
				WBS.central.render(document.body);
				WBS.central.hide();
				WBS.central.show();
	}, elCell);	
		YAHOO.util.Event.addListener(elCell, 'mouseout', function(e, oSelf){
				WBS.central.hide();
	}, elCell);	
};
...
/**
 * Formats cells in Columns of type "editLegislacao".
 *
 * @method formatEdit
 * @param elCell {HTMLElement} Table cell element.
 * @param oRecord {YAHOO.widget.Record} Record instance.
...
WBS.reloadtable = {
	success:function(o){
		//treelateral.removeChildren(treelateral.getNodeByProperty())	
	},
	failure: function(o){
		alert('ERRO');
	}
...
WBS.renewTree = function(elTree){
}

WBS.rmove = function (id) {
	if (confirm('Deseja RETIRAR esse ítem - e seus descendentes - do fluxo?')){
		
		WBS.kebra = id.split('_');
		t = WBS.kebra.length - 1;
		id1 = WBS.kebra[t];
		WBS.IDs[id1] = false;
		WBS.upTable[0].reloadTable();
		if(treelateral.getNodeByProperty('ID',id)) { 		
			var cObj = YAHOO.util.Connect.asyncRequest('GET', "geraconteudo.php?f=WWxjNWEyUlhlR3hqZVRsMFlqSlNabVJIVm5wa1IxVjJZMjFXZEdJeldteGphVFYzWVVoQlBRPT0=&id="+id+"&tipo=remove", null);
			supera = treelateral.getNodeByProperty('ID',id);
			if (supera.isRoot()) { 
				supera = supera;// alert('root'); 
			} else { 
				supera = supera.parent;// alert('naoroot'); 
			};
			supera.collapse();//
			treelateral.removeChildren(supera);
			supera.expand();
		} else {
			var cObj = YAHOO.util.Connect.asyncRequest('GET', "geraconteudo.php?f=WWxjNWEyUlhlR3hqZVRsMFlqSlNabVJIVm5wa1IxVjJZMjFXZEdJeldteGphVFYzWVVoQlBRPT0=&id="+id+"&tipo=removecriterio", null);
		}
	}
}
/**
...
YAHOO.widget.DataTable.formatCheckCond = function(el, oRecord, oColumn, oData) {
  ...
  var elTr = el.parentNode.parentNode;
  if (WBS.IDs){
	  if (WBS.IDs[oData]) { 
		if (WBS.MID) {
			el.innerHTML = '<div class="icon-locked" onClick="WBS.rmove(\''+WBS.MID+'_'+WBS.RowID+'\');">&nbsp;</div>';
		} else {
			el.innerHTML = '<div class="icon-locked" onClick="WBS.rmove(\''+WBS.RowID+'\');">&nbsp;</div>';
		}
		YAHOO.util.Dom.setStyle(elTr, "background-color","#FFEEDD") ;
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-asc');
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-desc');
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-odd');
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-even');
	  } else {
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-asc');
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-desc');
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-odd');
		YAHOO.util.Dom.removeClass(elTr, 'yui-dt-even');
		  YAHOO.util.Dom.removeClass(el, "td_icon-locked");
		  YAHOO.util.Dom.setStyle(elTr, "background-color","FFFFFF");
		 el.innerHTML = "<input type=\"checkbox\"" + bChecked +" class=\"" + YAHOO.widget.DataTable.CLASS_CHECKBOX + "\" id="+oData+">";
	  }
	  //alert('id: '+WBS.IDs[oData]);
	  //alert('data:'+oData);
  }
			/* 	var elTr = el.parentNode;
	if (oData < 0){	 
...
YAHOO.widget.DataTable.formatMark = function(el, oRecord, oColumn, oData) {
	Dom = YAHOO.util.Dom;
  var bChecked;
  var elSelRows = this.getSelectedRows();
  var elId = oRecord._sId;
  ...
  if (WBS.IDs[oData]) { 
	//el.innerHTML = 'OK';
	//elTr.style.backgroundColor = 'FF0000';
	Dom.addClass(elTr, 'mark');
	//alert(elTr.style.backgroundColor);
	//YAHOO.util.Dom.setStyle(elTr,"background-color","#FF0000");
  } else {
	  Dom.removeClass(elTr,'mark');
	//  elTr.style.backgroundColor = '';
	// YAHOO.util.Dom.setStyle(elTr,"background-color","");
	// el.innerHTML = "<input type=\"checkbox\"" + bChecked +" class=\"" + YAHOO.widget.DataTable.CLASS_CHECKBOX + "\" id="+oData+">";
  }
