function wsCTextEditor(name, html, width, height, buttons, readonly)
{
        this.name = name;
	this.html = html;
	this.width = width;
	...
	this.setHTML = wsCTextEditorSetHTML;
	this.addHTML = wsCTextEditorAddHTML;
	this.show = wsCTextEditorShow;
	this.getHTML = wsCTExtEditorGetHTML;
	this.clear =wsCTextEditorClear;
	
	// Private
	this.updateHTML =wsCTextEditorUpdateHTML;
...
function wsCTextEditorSetHTML(html)
{
	this.html = html;
	this.updateHTML();
...
function wsCTextEditorUpdateHTML()
{
	if (document.all)
    {
        frames[this.name].document.body.innerHTML = this.html;
	}
    else
    {
        document.getElementById(this.name).contentWindow.document.body.innerHTML = this.html;
	}

}


function wsCTextEditorAddHTML(html)
{
	this.html += html;
	this.updateHTML();
}
...
function wsCTextEditorShow()
{
	writeRichText(this.name, this.html, this.width, this.height, this.buttons, this.readonly);
}


function wsCTExtEditorGetHTML()
{
   	if (document.all)
    {
        return frames[this.name].document.body.innerHTML;
	}
    else
    {
...
function writeRTE(rte, html, width, height, buttons, readOnly) {
	...
	document.writeln('<iframe id="' + rte + '" name="' + rte + '" width="' + width + 'px" height="' + height + 'px" class="wsEditorText" style="display:inline;background-color:#8890A1;"></iframe>');
	if (!readOnly) document.writeln('<br /><p style="text-align:left;font-size:8pt" ><input type="checkbox" id="chkSrc' + rte + '" onclick="toggleHTMLSrc(\'' + rte + '\');" />&nbsp;View Source</p>');
